...
const ACTIONS: { label: string; price: string; hint: string; free?: boolean }[] = [
	...
	{
		label: "Генерация по анамнезу",
		price: `${PRICES.anamnesis} ₽`,
		hint:  "Несколько идей под ваш профиль",
	},
	{
					...
					<Reveal className="relative overflow-hidden rounded-3xl border border-amber-500/35 p-7">
						...
						<div className="relative">
							...
							<p className="mt-3 text-sm leading-relaxed text-stone-300">
								Дарим {PRICES.welcomeBonus} ₽ при регистрации — этого
								хватает на первый полный AI-анализ. Попробуйте сервис, не
								доставая кошелёк.
							</p>
							...
							<p className="mt-3 text-center text-xs text-stone-500">
								Вход через Google или magic-link, без пароля
							</p>
						</div>
					</Reveal>
					...
					>
						...
						<div className="mt-6 rounded-2xl border border-stone-800/60 bg-stone-950/50 p-4">
							...
							<p className="mt-1.5 text-xs leading-relaxed text-stone-500">
								От 100 ₽ через ЮKassa, с чеком. Удобные пресеты:
							</p>
							<div className="mt-3 flex flex-wrap gap-2">
								{TOPUP_PRESETS.map((amount) => (
									<span
										key={amount}
										className="rounded-xl border border-stone-700/70 bg-stone-900/70 px-3.5 py-1.5 text-sm font-semibold tabular-nums text-stone-200 transition hover:border-stone-600 hover:bg-stone-800/70"
									>
										{amount} ₽
									</span>
								))}
